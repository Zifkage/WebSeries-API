Feature: Retrive User by ID
  ...
  Scenario: Retrive No-existing User
    If the client sends a GET request to /api/users/:userId with an ID that does not exist, it should receive a response with a
    4xx status code.

    When the client creates a GET request to /api/users/8888
  ...
  Scenario: Retrieve Existing User
    Given the client creates a POST request to /api/users/
    And attaches a valid Create User payload
    And sends the request
    And saves the response text in the context under :userId
    When the client creates a GET request to /api/users/:userId
    And sends the request
